...
| **cursor-agent**（CLI 子代理，非 Cursor IDE） | precise；无需 env 标记 | `~/.cursor/projects/<encodedCwd>/agent-transcripts/<uuid>/<uuid>.jsonl`（`{role,message}` 行 + `turn_ended` 事件）；encodedCwd = cwd 盘符冒号与分隔符全 `→ '-'`（`C:/Users/qinyi` → `C-Users-qinyi`，正向编码比对，数字目录名跳过）；format 串 `cursor-agent-transcript-jsonl`（daemon 解析器注册键逐字一致，坑 cursor-agent-transcript-report-pipeline） | 目录名即 cwd 编码，天然精确归属；token 不落盘 → usage 未知（平台 FR-03 不伪造） |
