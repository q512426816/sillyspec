...
| **zcode** | precise；env `ZCODE_*`（ZCODE_APP_VERSION/ZCODE_ENV/ZCODE_RUNTIME_ENV 任一） | `~/.zcode/cli/rollout/model-io-sess_<id>.jsonl`（subagent 文件名带 `sess_subagent_agent_` 前缀） | 无 cwd 字段，env 门控即归属依据（同一 CLI 实例） |
