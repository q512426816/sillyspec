...
> **源文件**：`src/stages/doctor.js`
> **阶段定位**：检查 SillySpec 配置、构建环境和外部依赖
> **类型**：辅助阶段（auxiliary，无活跃变更时也可执行）
> **全局角色 persona**：无（doctor 是辅助阶段，不注入 persona）
> **全局护栏 _globalGuardrails**：无（仅有 CLI 统一铁律，见 [README.md](./README.md)）
> **步骤总数**：6

> 📌 本文档展示的是**每个 step 的 prompt 模板原文**。agent 实际收到的提示词 = `outputStep` 注入的 header + prompt 正文（占位符已替换）+ 铁律（仅首步）+ `--done` 命令模板。doctor 为辅助阶段，不注入 persona / 模块上下文 / 完成契约；注入细节见 [README.md](./README.md)。

---

## Step 1/6：SillySpec 内部检查
...
````markdown
运行 SillySpec 内部检查。逐项执行以下命令并汇总结果：
...
````markdown
检查项目构建环境。先探测项目使用的构建工具，再逐项检查可用性。
...
**本步出现的运行时占位符**
- `<project>` → 当前项目名（`basename(cwd)` 或 db 项目名）；本步出现在路径 `.sillyspec/docs/<project>/modules/_module-map.yaml`
- （prompt 中的 `<module-id>` 是路径示例的字面占位符，非 CLI 运行时替换；完整映射见 [README.md](./README.md)「占位符总表」）
...
````markdown
检查决策知识库（knowledge/decisions/）的待复核状态（advisory，不阻断）。
