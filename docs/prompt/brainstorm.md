...
````markdown
通过对话探索需求细节、分析原型（如有）、判断拆分/批量、并对歧义点做需求澄清 Grill。本步骤合并了原有的对话探索、原型分析、需求范围评估、需求澄清 Grill 四个环节——按下方 A→D 顺序按需执行，能一次问清的不要拆成多轮。
...
````markdown
展示完整设计方案供用户确认；如适合可视化，顺带生成 HTML 原型。
