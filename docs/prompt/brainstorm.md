# brainstorm（头脑风暴）阶段提示词

> **源文件**：`src/stages/brainstorm.js`
> **阶段定位**：探索需求、分析技术方案、识别风险
> **类型**：主流程阶段
> **全局角色 persona**：资深架构师（15 年经验的系统架构师，先理解业务本质再设计技术方案，决策附理由、列 trade-off）— 逐字文案见 [README.md](./README.md) persona 表。
> **全局护栏 _globalGuardrails**：无（仅有 CLI 统一铁律，见 README）。
> **步骤总数**：8
...
````markdown
加载项目现有上下文，理解代码结构和约定；并做一次早期规模筛查，判断小变更是否该直接走 quick。
...
- **必须生成**（布局/结构/流程级变化）：
  - 新增页面、视图、路由
  - 页面布局变化：区域增删、骨架重排、信息层级调整（如单栏改多栏、列表改看板）
  - 新交互流程：多步流程、跨页流转、状态驱动的界面变化
  - 用户提供了截图/设计稿要求按其改造（原型即对照基准）
- **建议生成**（组件级变化、页面骨架不变）：
  - 表单重构、列表列调整、弹窗/抽屉流程等局部组件变化
  - 拿不准时默认生成——宁多勿漏，原型在 execute 阶段可直接对照复用
- **跳过**（无需生成，但必须声明原因）：
  - 文案、颜色、间距、字号、图标等样式微调
  - 纯后端逻辑/CLI/配置/文档，无任何界面变化

### 生成要求
生成一个独立的 HTML 文件（内联 CSS + JS），保存到：
`{SPEC_ROOT}/changes/<change-name>/prototype-<名称>.html`（目录不存在则先创建）
- 单文件，浏览器直接打开
- 高保真：布局/组件/交互流程按真实效果呈现，后续 execute 阶段可直接对照复用
- 与项目代码风格一致：优先复用项目现有技术栈/组件库/设计 token（先查 scan 文档与现有前端代码），不另起风格

### 铁律 — 必须等待用户确认设计
- 展示完整设计方案后必须暂停等用户确认，**不要自己说"设计已充分确认"然后推进**（命令由 CLI 在下方注入）
- 原型生成或跳过声明必须在暂停**之前**完成——暂停是本步骤最后动作，用户确认后直接进入写 design.md，不会回头补

### 输出
完整设计方案 + 变更名 + 原型文件路径 或 跳过原因（二选一必填；跳过时写明分级依据，用户可当场否决要求补）
...
7.5. **生命周期契约表**（涉及以下关键词时必填，否则可省略）：
   ...
   - 确实不涉及 → 写**紧邻豁免短语**（照抄其一）：「不涉及生命周期契约」「不适用 lifecycle contract」「生命周期契约：无/N/A」
     ——「本变更无需 lifecycle 事件」「lifecycle 状态无变化」这类宽写法**不被识别**（否定词必须紧邻「生命周期契约/lifecycle contract」）
...
10b. **非功能生命周期**（进程/资源/清理面——2026-09-22 R7 dogfood 复盘新增，41 孤儿 watcher
泄漏实证：设计审查惯于覆盖 API/通道/语义面，「死亡面」无人审）：
   - 本变更是否引入长驻进程/后台任务/持有外部资源（文件锁/监听句柄/子进程）？
   - 引入 → 逐项写自灭条件（谁负责杀、何时死、父环境消失后靠什么死——不得只依赖 spawn 侧
     环境变量；参考三闸模式：首拍终态即退/外部资源蒸发连续 N 拍退/硬寿命帽）
   - 不涉及 → 风险登记表加一行「无长驻进程/外部资源，生命周期面不适用」显式留痕

11. **决策追踪**（如存在 Grill/重大决策）：
   - 列出当前版本 D-xxx@vN 决策 ID
   - 说明每个 D-xxx@vN 被哪些 FR-xxx / 设计章节覆盖
   - 标注仍未解决的 D-xxx@vN 或剩余风险
   - **多裁定组合推演**（2026-09-22 R7 dogfood 复盘新增：升厚×断点续两裁定各自正确、组合
     出归档死锁的实证）：≥2 条相互约束的裁定（状态升降/回退路径/失败语义类）时，自审必须
     做组合状态表推演——各裁定的状态叉乘 × 关键子步（如归档/重入）逐格过一遍，死锁格写
     进风险登记与调和方案；无组合面（裁定互不作用）显式记「无组合约束」一行
12. **自审**（AI 对自身设计的校验）
...
````markdown
默认执行 Design Grill，对已经写出的 design.md 做交叉审查。
...
{REVIEW_JSON_CONTRACT}
  ...
  **审查经济学三律（2026-09-20 对撞实验实测驱动——前置阶段 23 分钟审查等待的结构性收口）**：
