...
> 源：`src/run/prompt.js` `outputStep` 铁律注入段（约 L540-548）。正向化：否定式（不得/不要）改正向指令 + 理由附着，弱模型遵从度更高。

- 文档优先：代码产出必须先有对应的设计/规范文档支撑。
- 聚焦本步骤：只执行本步骤描述的操作并完整做完；自行扩展或跳步会破坏状态机推进，后续步骤再做后续事。
- 已完成步骤视为只读：需修订用 `sillyspec run <stage> --reopen --from-step N` 回退重做，直接回头改会让进度记录与产物脱节。
- CLI 子命令以本 prompt 或上一条 `--done` 输出的字面为准；不确定时停下问用户，猜测命令会让状态机推进到错误位置。
- 本步骤产物落盘后立即执行 prompt 末尾的 `--done`：CLI 据此校验产出并推进状态机；不跑则进度永远停在本步。
- 变更目录改名用 `sillyspec change-rename <旧名> <新名>`：mv/rename 会漏改进度库引用，导致变更失联。
...
| `{REVIEW_TIER}` | 审查分级：`self`（当前 agent 自审）或 `independent`（强制独立子代理 + review.json）+ 仪式档位注入（S0-S3 菜单块，`renderCeremonyTierInjection`）。由 `review-tier.js` 的 `classifyReviewTier` 委托 `ceremony-tier.js` 定价（三轴取封顶；plan_level 仅编排不再独自决定 independent，2026-09-18-ceremony-risk-pricing） | brainstorm / plan / execute 的 review 步 |
