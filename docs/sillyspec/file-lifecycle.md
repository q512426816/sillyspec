...
阶段 3 步：step1 noAI `doctorRunDiagnostics`（全量诊断十一维 + renderDoctorSummary 渲染 + `.runtime/doctor-diagnosis.json` 落盘）；step2 修复决策与执行；step3 汇总。顶层 `sillyspec doctor`（非 --json）直跑诊断渲染（只读零副作用）；`--json` 走 formatDoctorDiagnosis 既有机器接口。
