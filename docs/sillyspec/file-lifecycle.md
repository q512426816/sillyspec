...
| archive | 5 | 辅助阶段；第 4 步必须带 `--confirm`，由 `run/complete-handlers.js`（`archiveChangeDirectory`）移动目录并注销 active change；移动前硬校验 `plan.md` 存在，移动后校验 `design.md`/`module-impact.md`；step2 `extract-module-impact`（**不改名**）2026-08-13 起改为**终审**——module-impact.md 不再由 archive 生成，改由 plan review_plan 步生成首版（large）、execute 各 Wave 主代理汇总更新、verify 核对、archive 终审（活文档全程演化，见下「module-impact.md 多阶段」） |
