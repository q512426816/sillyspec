...
updated_at: 2026-08-15T20:30:00+08:00
---
...
| brainstorm | 8 | 独立包含”写设计文档并自审”（第 6 步）、”Design Grill 交叉审查”（第 7 步）、”生成规范文件”（第 8 步，原名”用户确认并生成规范文件”，已去确认门控——设计在第 5 步「分段展示设计」确认过，末步直接生成后展示摘要）；第 2 步加载上下文时含早期规模筛查（明显小变更建议走 quick）；2026-08-15 D-7 起该步另注入 `{SCAN_STALENESS}` 漂移事实（`src/scan-staleness.js` CLI 算 source_commit vs HEAD：落后 ≥50 commit 或 ≥14 天 → stale 提示不盲信 + 刷新指引；fresh/unknown/绿地各有降级文案，advisory 不阻断）；完成时按 design.md frontmatter `scale` 分叉产物（large→四件套进 plan / small→仅 design.md 进 quick）；**`validateBrainstormOutputs` 经 contract `condition(scale≠small)` 豁免 proposal/requirements/tasks 三规则**（2026-08-07 修复：末步 small 指引只写 design.md 后 validator 四件套全 error 撞墙的矛盾；fail-safe：读不到 scale → 四件套全要求，保守走重流程） |
