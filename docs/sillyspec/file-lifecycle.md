...
| 阶段 | 当前步骤数 | 备注 |
|---|---:|---|
| scan | 11 | 辅助阶段；step 2 后会按项目动态展开 `perProject` 步骤；第 10 步「Extract Project Knowledge」写入 `knowledge/`；**scan diff 增量命令**（2026-08-16，change 2026-08-16-scan-diff-command）：`sillyspec scan diff [--base <commit>] [--full] [--report]` 用 git diff 算 source_commit..HEAD 漂移清单（A/D/M/R 四分类，`src/scan-diff.js` 复用 `matchFilesToModules` 按 module-map v2 paths 归模块），agent 按清单定点补而非全量重扫 |
| brainstorm | 8 | 独立包含”写设计文档并自审”（第 6 步）、”Design Grill 交叉审查”（第 7 步）、”生成规范文件”（第 8 步，原名”用户确认并生成规范文件”，已去确认门控——设计在第 5 步「分段展示设计」确认过，末步直接生成后展示摘要）；第 2 步加载上下文时含早期规模筛查（明显小变更建议走 quick）；2026-08-15 D-7 起该步另注入 `{SCAN_STALENESS}` 新鲜度提示（`src/scan-staleness.js` CLI 算 source_commit vs HEAD 落后数：≥50 commit 或 ≥14 天 → needs-refresh——提示核对/重扫而非判文档失效，文档引用是否失效由 docs check 判定；2026-08-16 ql-20260816-009-fb44 修正语义）；fresh/unknown/绿地各有降级文案，advisory 不阻断）；完成时按 design.md frontmatter `scale` 分叉产物（large→四件套进 plan / small→仅 design.md 进 quick）；**`validateBrainstormOutputs` 经 contract `condition(scale≠small)` 豁免 proposal/requirements/tasks 三规则**（2026-08-07 修复：末步 small 指引只写 design.md 后 validator 四件套全 error 撞墙的矛盾；fail-safe：读不到 scale → 四件套全要求，保守走重流程） |
