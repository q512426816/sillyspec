...
## 附录 B：批 3（artifact-prefill）对表数据——2026-09-19 归档后直核（D-004 兑现）

| 指标 | 基线（旧版全仪） | 批3 实测 | 备注 |
|---|---|---|---|
| 主会话请求 | 166 | 180* | *含主代理直连的全部 main 线会话（多 quick 线混入，口径偏宽） |
| 主会话 token | 39.2M | 20.9M* | **-47%**（同上口径偏宽下的下限估计） |
| 评审子代理 | 179 req / 22.4M | 实现子代理 578 req / 68.7M | 实现子代理是流程设计（任务并行派发），非评审轴——评审轴本变更仅 1 轮独立验收（35 req/1.8M） |
| 摩擦 | 16 | friction 升档触发 1 次（S1→S2） | 定价引擎首战 |
| 独立评审轮 | 2 轮 | Grill self（S1 轻仪）+execute 1 轮（升档后） | 评审轴 -75% 轮次 |

归因注记：主会话 20.9M 与基线 39.2M 的口径差（批3 含主代理 direct 编辑实现）——真实降本主要由 S1 轻仪（零 Grill 独立子代理）与注入瘦身（分叉后单步上下文减）贡献；精确归因需下个自然变更复测。
