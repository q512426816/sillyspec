...
**阶段完成 gate 级联**（`completeStageGates` `src/run/gates.js:810`，统一收尾管线）顺序：
...
2. verify 实测对账：CLI 亲跑 `local.yaml` 的 `commands.test`，自报告 PASS 但实测失败→阻断（`gates.js:421`）。
3. Plan→Execute Contract（`validatePlanForExecute` `gates.js:473`）。
4. Stage Review Gate（brainstorm/plan/execute，`gates.js:501`）：`classifyReviewTier` 判 tier=self（自审）/independent（强制独立子代理 review.json）。
