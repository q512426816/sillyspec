...
**转换合法性**由 `checkTransition(fromStage, toStage)` 仲裁（`src/stage-contract.js:954`），契约转换图（`src/stage-contract.js:954`）：
...
| 门 | 触发 | 阻断 | 依据 |
|---|---|---|---|
| 转换门 `checkTransition` | 阶段跳转不符合 allowedFrom / scan failed | `exit(1)` | `src/stage-contract.js:954` |
...
**worktree-apply —— 跨仓 task 合并回主干**（`applyWorktree` `src/worktree-apply.js:748` 起，变更文件列表经 `filterDeliverableFiles` `src/worktree-apply.js:55`）：跨仓 task no-op 校验 → meta 校验 → 变更文件列表（`filterDeliverableFiles` 排除 `.sillyspec/`）→ allowList 校验（从 task 卡 `allowed_paths` 读）→ `assessApplyRisk` 风险审计（SAFE/WARNING 自动 apply 到 main）。
...
| `.bak` 主动写时机 | `db.js:96`/`progress.js:825` 注释称"写前自动备份为 .bak" | `_write`/`transaction` 路径未见主动 copy 到主 `.bak`（主 `.bak` 实际由 `_openWithFallback` 恢复时使用 + import 的 `.pre-import-*.bak` 产生） | 已订正（见 ql-20260814-003）：4 处描述统一为「node:sqlite 提交即持久化、`.bak` 恢复是向后兼容兜底」 |
