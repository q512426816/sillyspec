...
**转换合法性**由 `checkTransition(fromStage, toStage)` 仲裁（`src/stage-contract.js:776`），契约转换图（`src/stage-contract.js:776`）：
...
| execute deps 门 | worktree `depsStatus` 未达标 | step blocked + `exit(1)` | `src/run/gates.js:304` |
...
**阶段完成 gate 级联**（`completeStageGates` `src/run/gates.js:901`，统一收尾管线）顺序：
...
3. Plan→Execute Contract（`validatePlanForExecute` `gates.js:563`）。
4. Stage Review Gate（brainstorm/plan/execute，`gates.js:590`）：`classifyReviewTier` 判 tier=self（自审）/independent（强制独立子代理 review.json）。
