...
- 最强资产被埋没：`collectDocRefs` / `resolveCandidates` / `validateRefLines`（`src/docs-check.js:198` 起）已实现「行级+符号级核验+自动重锚（classifyFix）」，但 ① 只认 .js/.mjs（P1b 后扩展）；② 未接进 scan 管线（P0 后已接）；③ 读工作树而非钉住 commit。
