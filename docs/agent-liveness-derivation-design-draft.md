...
| # | 问题 | 影响 | 处置 |
|---|---|---|---|
| E-01 | claude-code transcript 是否记录 permission 等待（CLI 内部交互可能不落 transcript） | 决定该 format 能否承诺 blocked | P1 前实证：本机制造确认等待，逐行查 transcript 尾部。**注记（2026-09-07）**：E-02/E-03/E-08 收敛后，本项成为日志推导 blocked 的唯一剩余候选，价值场景＝裸 claude CLI（托管 Claude 走第一方管线） |
