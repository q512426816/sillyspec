          ...
          try {
            ...
            if (escapes.length > 0) {
              issues.push({ type: 'editable-install-escape', name, fixable: false,
                detail: `worktree venv 含 ${escapes.length} 个 editable install 指向 worktree 外（${escapes.slice(0, 5).map(e => `${e.pkg}→${e.target}`).join(', ')}${escapes.length > 5 ? ' 等' : ''}）——生成链/后端命令会静默加载 worktree 外旧代码。轻量出口：跑生成链前设 PYTHONPATH=<worktree>/<项目源码根>（PYTHONPATH 优先于 .pth）；彻底修复：worktree 内重装（uv sync / uv pip install -e .）` });
            }
