...
⚠️ **「需求：」（= --req）就是 QUICKLOG 条目标题**——CLI 提取该字段、截到首个标点（，。；,;）、超 80 字再截断。写成完整需求长句会被截成语义不完整的状语前半段（如「在 X 时，…」标题只剩「在 X 时」），事后还得手改。一句话说清改了什么（如「登录限流修复——INCR 计数误清」），需求背景放「根因：/方案：」字段。单行旧形式的风险仅在「正文里再次出现顶层标签字样」——需求：/根因：/方案：/结果： 字样前导恰是空白或句末标点（。；！？）时，会被边界扫描误当真实字段边界（错位/缺字段被拒）——这正是四参数形式消灭的事故面。注意区分：根因块内以列表行形态出现的嵌套子字段（\`- 现象：\` \`- 根因：\` \`- 护栏：\` \`- 证据：\`）是**合法形态**——「- 」前缀不构成顶层标签、不参与拆分判定，顶层四字段边界不受影响（postmortem 用法见下方可选提示）。
