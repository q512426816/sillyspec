...
4. **增量落盘与中断接手指引**：每完成一个可见产出（代码/测试/文档），立即写盘并执行一次最小验证（如语法检查、单跑相关测试）。工作过程中如被 429/API 配额/会话中断，应在最终回复里输出「已完成清单」（含文件路径、测试命令、当前卡点），不要只输出结论——续跑时依据磁盘产物和该清单判断哪些已完成，避免重做已落盘的工作
   ...
   **输出纪律**：测试/安装/构建类长输出命令一律重定向落盘（\`> {SPEC_ROOT}/.runtime/logs/<名>.log 2>&1\`，CLI 已替换为绝对路径）后 \`tail -n 50\` 回看结论、按需 grep——勿把全量输出读进上下文（长输出逐轮回放是 execute 段 token 膨胀主源，R12 对撞实证均轮 134K vs 其他 181-234K）
