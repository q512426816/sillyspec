...
- **主文件超 500 行时 CLI 自动轮转，勿当数据丢失**：历史条目被整体挪到同目录 `QUICKLOG-<user>-<日期>.md`，主文件仅剩新条目（CLI echo `🔄 QUICKLOG 已轮转…`）。git diff 上主文件呈大幅删除是正常轮转——**勿从 git 恢复**（恢复会与轮转副本重复，2026-08-28 曾因此误判触发多余恢复提交）；提交时把轮转归档文件带上
