...
- **`sillyspec init`（CLI 命令）** — 安装命令模板/MCP 工具到项目（零交互，自动检测工具）。用法：`sillyspec init [--tool <名>] [--interactive] [--dir <path>] [--spec-dir <path>]`
- **`/sillyspec:init`（本 skill）** — 绿地项目的需求文档生成流程（深度提问 → REQUIREMENTS.md → ROADMAP.md → PROJECT.md），不写代码、不装依赖

两者用途不同、不冲突：可先用本 skill 搞清需求，再用 CLI `sillyspec init` 装工具。
