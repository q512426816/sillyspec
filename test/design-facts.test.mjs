...
console.log('\n=== 6. design-init CLI（骨架落盘接线）===\n')
{
  ...
  assert(gen.includes('| D-001@v1 | （待填覆盖点） | 待确认 (预填：核对后删本注) |'), '决策追踪表按当前版本预填（task-02 接线：行带来源注——D-003 预填≠结论，D-006 裁决改新格式）')
  ...
  assert(forced.includes('| D-001@v1 | （待填覆盖点） | 待确认 (预填：核对后删本注) |'), '--force 重生成后追踪表仍预填（带来源注新格式）')
