...
test('① draftAll 四件形态（标记+AGENT 槽+ledger 首版原文）+任务卡分岔', () => {
  ...
  assert.deepEqual(a.written, ['proposal.md', 'requirements.md', 'tasks.md'])
  assert.ok(!existsSync(join(changeDir, 'tasks')), '默认 thin 零任务卡')
  for (const f of ['proposal.md', 'requirements.md', 'tasks.md']) {
...
test('⑥ 薄跑道会话内 .sillyspec 写入=仅例外裁决（真 CLI harness 验产物面）', () => {
  ...
  assert.deepEqual(snapshotBefore, ['flow-state.yaml', 'proposal.md', 'requirements.md', 'tasks.md'], '产物面=四件+状态（零手写额外文件）')
