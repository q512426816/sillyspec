...
test('④ 本机真实布局只读探测（96+ 份存量，最新的一份在窗口内即命中）', { skip: (() => {
  // 全量 runner（run-tests.mjs）把 HOME/USERPROFILE 隔离到 suiteTmp——homedir() 恒指隔离目录，
  // 真实布局探不到（2026-09-25 推送门实证的结构性红）。显式锚真实 home；机器不在（CI/他机）跳过。
  return existsSync('C:/Users/qinyi') ? false : '真实 home 不在本机（CI/他机跳过）'
})() }, () => {
  ...
  const entries = detectAgentLogEntries({
    ...
    homeDir: 'C:/Users/qinyi', now: Date.now(), windowMs: 365 * 24 * 3600_000,
  })
  const hits = entries.filter(e => e.harness === 'cursor-agent')
