 ...
 *      stages.archive=completed + 步骤全 completed（含从未初始化时按定义补种——步数随 registry 单一真相，P0-4 并步后 3 步）
...
console.log('--- ① unregisterChange 收尾参数：未初始化 archive 阶段按定义补种（P0-4 三步） ---')
{
  const d = mkRepo('u1')
  ...
  assert(names.length === 3, `archive 步骤名取自 registry（P0-4 并步后 3 步，实得 ${names.length}）`)
  ...
  assert(t.steps.length === 3 && t.steps.every(s => s.status === 'completed'), `3 步全 completed（实得 ${t.steps.length} 步）`)
  cleanup()
}
...
console.log('--- ② archiveChangeDirectory 自愈路径（手动搬目录后）走完整终态 ---')
{
  ...
  assert(t.steps.length === 3 && t.steps.every(s => s.status === 'completed'), `归档步骤全 completed（${t.steps.filter(s => s.status === 'completed').length}/${t.steps.length}）`)
  cleanup()
}
...
console.log('--- ③ doctor --cleanup-ghosts：手动归档型收尾，真丢失型 status-only ---')
{
  ...
  assert(tm.status === 'archived' && tm.currentStage === 'archive' && tm.steps.length === 3 && tm.steps.every(s => s.status === 'completed'),
