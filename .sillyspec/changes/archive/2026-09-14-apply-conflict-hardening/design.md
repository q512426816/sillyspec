...
| 修改 | .sillyspec/docs/sillyspec/modules/worktree.changelog.md | task-04：模块卡登记配套 sidecar（worktree 变更条目） |
| 修改 | .sillyspec/docs/sillyspec/modules/core-engine.changelog.md | task-04：模块卡登记配套 sidecar（core-engine 变更条目） |
| 修改 | test/worktree-apply-rescue.test.mjs | task-04：A1/A2/A4 三断言改过滤 # 注释行后计数+A1 补指引行断言（task-01 rescue 指引行的预期契约 fallout，按 related_tests 归本任务） |
