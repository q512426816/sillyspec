...
> author: zcode-fr-l2
> created_at: 2026-09-20 08:20:00（骨架由 `sillyspec verify-probes --change <变更名> --init` 生成）
...
结论枚举：`PASS WITH NOTES`——NOTE=quality-scan 记录时序缺口（工具面，实测事实由移交行承载）。原判：五任务（薄三厚二）实现+证据双核：fr-index 23/23直测（矩阵/条目行/digest/指针/省略/GWT/回填七组）+全量546绿（对面在途守卫测试单跑9/9不计）+lint 698绿；S1 execute自审pass五项清单；范围修正（薄→厚D-003@v2）全程留痕（把尖括号占位整体替换为 PASS / PASS WITH NOTES / FAIL 之一；一句话理由写在枚举后同行或下一行）
...
| 类型 | 条目 | 复跑/验收条件 |
|---|---|---|
| other | quality-scan 实测记录未落（--done 亲测替代扫描时序不可得，X-01 已知缺口）——npm test 546绿+定向23/23+lint 698均实跑 | 记录落点修复后 facts 刷新自动闭环 |
...
- ⚠️ task-02: 模块目录（src/run、src/stages）递归未找到测试文件（含 co-located tests/）
...
|---|---|---|---|---|
| 矩阵命中 FR 块 decisions 正确；无矩阵条目省略行 | `test/fr-index-l2.test.mjs` | — | covered | `test/fr-index-l2.test.mjs`（七组23/23实跑：矩阵/条目行/digest/指针/省略/GWT/回填） |
| 条目行/digest.decisions/文件头指针逐条断言绿 | `test/fr-index-l2.test.mjs` | — | covered | `test/fr-index-l2.test.mjs`（七组23/23实跑：矩阵/条目行/digest/指针/省略/GWT/回填） |

**task-02**
| acceptance 条目 | 归属测试文件 | 关键词命中（提示，命中≠判定） | 判定 | 证据 |
|---|---|---|---|---|
| 注入渲染含 D 锚；decisions 空条目不渲染锚段 | `test/fr-index-l2.test.mjs` | — | covered | `test/fr-index-l2.test.mjs`（七组23/23实跑：矩阵/条目行/digest/指针/省略/GWT/回填） |

**task-03**
| acceptance 条目 | 归属测试文件 | 关键词命中（提示，命中≠判定） | 判定 | 证据 |
|---|---|---|---|---|
| 五组断言绿；全量 npm test 绿 | `test/fr-index-l2.test.mjs` | — | covered | `test/fr-index-l2.test.mjs`（对应组断言实跑） |

**task-04**
| acceptance 条目 | 归属测试文件 | 关键词命中（提示，命中≠判定） | 判定 | 证据 |
|---|---|---|---|---|
| GWT 行正确归属场景；条目块渲染与截断符合契约 | `test/fr-index-l2.test.mjs` | — | covered | `test/fr-index-l2.test.mjs`（七组23/23实跑：矩阵/条目行/digest/指针/省略/GWT/回填） |

**task-05**
| acceptance 条目 | 归属测试文件 | 关键词命中（提示，命中≠判定） | 判定 | 证据 |
|---|---|---|---|---|
| 回填补正文；二跑零新增（幂等）；无归档/标题不匹配 → 警告不阻断 | `test/fr-index-l2.test.mjs` | — | covered | `test/fr-index-l2.test.mjs`（七组23/23实跑：矩阵/条目行/digest/指针/省略/GWT/回填） |
...
- ℹ️ 清单无 .java 文件（另有 3 个非 Java 清单文件不在探针 9 扫描面）
#### 探针 10：预填注清零（error 门）
...
- ❌ 预填注未清 2 处（error 门——verify 完成前须逐槽核对后删注）：
...
|---|---|---|---|---|
| D-001@v1 | FR-01、FR-02 | task-01、task-02 | <待填：证据回指> | <待填> |
| D-002@v1 | FR-01、FR-02 | task-01 | <待填：证据回指> | <待填> |
| D-003@v2 | FR-01、FR-02、FR-03、FR-04 | task-04、task-05 | <待填：证据回指> | <待填> |
