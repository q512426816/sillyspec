...
- [x] task-05: 配套四小修——skip 跨仓档位 / adopt 勾选两层 / probe7 多根 / design 无段头缺口 (depends_on: task-01)
- [x] task-06: prompt/清单新增条目 + verify prompt 封顶提示 + 文档镜像再生（_extract.mjs） (depends_on: task-02)
- [x] task-07: 测试补全——NEW test/pass-eligibility.test.mjs + 8 个既有测试文件就近断言 (depends_on: task-01, task-02, task-03, task-04, task-05, task-06)
- [x] task-08: Wave 步骤完成度门——assertWaveTasksComplete（Wave N --done 前校验本 Wave checkbox 全勾，fail-closed）+ NEW test/wave-task-complete-gate.test.mjs (depends_on: task-05)
