...
| 操作 | 文件路径 | 说明 |
|---|---|---|
| 新增 | NEW:src/scope-audit.js | 纯函数 computeChangeScopeAudit + renderScopeAuditTable（人类可读渲染）+ collectNumstatByPath（numstat/wc-l/BIN 三档采集，quick 与 full-flow 共用） |
| 新增 | NEW:test/scope-audit.test.mjs | 三态判定/归属分档/行数三档/降级路径/并行会话退栈不进表 夹具测试 |
...
| 修改 | docs/sillyspec/platform-interface-map.md | 连带机械修复：task-03 在 index.js 插入命令路由致 6 处 index.js:<行号> 锚点漂移（+6/-6 平移，无内容改动；不修则 doc-ref-check.test.mjs 红；ql-20260816-015 同款先例——execute 期补声明） |

### 不修改文件（execute 阶段）
- .sillyspec/docs/sillyspec/modules/_module-map.yaml —— 归档阶段由 sync-module-docs 步按实际模块归属更新（archive 流程职责），execute task 无改动面
