...
> created_at: 2026-09-20 21:30:00（骨架由 `sillyspec verify-probes --change <变更名> --init` 生成）
...
结论枚举：`PASS WITH NOTES`——四任务实现+25/25直测+554/0全量+lint绿；S2执行验收三轮闭环（3+2真缺陷全修，终版13项全过）；NOTES=①lint快照31s正常②R5c移交：design.md补auto-link收窄句（verify期本NOTE承载）（把尖括号占位整体替换为 PASS / PASS WITH NOTES / FAIL 之一；一句话理由写在枚举后同行或下一行）
...
| 类型 | 条目 | 复跑/验收条件 |
|---|---|---|
| other | design.md 补 auto-link 收窄句+文件表 classify 措辞回写（S2 验收 R5c 移交——语义注记不影响行为） | 下次触碰该 design 时顺手回写，或接受现状（终态语义已在代码注释+review.json 全录） |
...
|---|---|---|---|---|
| 写入后条目含待复核行，digest.needssReview 透传，幂等二跑零新增 | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |
| 承接翻链后待复核行清除（filter 断言） | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |
| 注入行带 ⚠️ 标注 | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |

**task-02**
| acceptance 条目 | 归属测试文件 | 关键词命中（提示，命中≠判定） | 判定 | 证据 |
|---|---|---|---|---|
| 夹具：门禁过后 FR 入索引+决策蒸馏+目录进 archive/ + DB 注销 | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |
| 目录已在 archive/（自愈态）→ unregister 补账不留 skipped | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |
| 所有权拒 → skipped 带 owner，无副作用 | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |

**task-03**
| acceptance 条目 | 归属测试文件 | 关键词命中（提示，命中≠判定） | 判定 | 证据 |
|---|---|---|---|---|
| 门禁 fail 时不蒸馏（D-002 时序断言） | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |
| 纯 quick 无 linked 真变更 → 蒸馏跳过、机械件照跑 | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |
| 机械件异常不影响 quick 完成退出码 | `test/quick-asset-tail.test.mjs` | — | covered | `test/quick-asset-tail.test.mjs`（25/25 实跑，7 组含机械件/蒸馏尾/needs_review 四态） |

**task-04**
| acceptance 条目 | 归属测试文件 | 关键词命中（提示，命中≠判定） | 判定 | 证据 |
|---|---|---|---|---|
| 全部断言绿 + 全量 npm test 绿 | `test/quick-asset-tail.test.mjs` | test（`test/quick-asset-tail.test.mjs`） | covered | `test/quick-asset-tail.test.mjs:12`（test） |
...
- ✅ API parity check passed: 3 backend endpoints (live [scan-root 4] + artifact 0), 0 frontend calls [scope: change-diff (2 files @ scan-root)] | 3 backend endpoints unused by frontend
...
|---|---|---|---|---|
| D-001@v1 | FR-01 | task-02 | <待填：证据回指> | <待填> |
| D-002@v1 | FR-01 | task-02 | <待填：证据回指> | <待填> |
| D-003@v1 | FR-02 | task-01 | <待填：证据回指> | <待填> |
| D-004@v1 | FR-01 | task-02 | <待填：证据回指> | <待填> |
| D-005@v1 | FR-01、FR-02、FR-03 | task-03、task-04 | <待填：证据回指> | <待填> |
