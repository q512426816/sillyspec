...
- `runAutoMode(...)`（`src/run/command.js:1140`）：自动模式，连续跑
...
- 审批链路：`GET /api/changes/{name}/approval` 查询（`sync.js:576`，`checkApproval`）+ `approve` / `reject`
  入口（`sync.js:1132` / `sync.js:1137`，共用 `_submitApproval`，端点契约 TBD-hub-api）。
