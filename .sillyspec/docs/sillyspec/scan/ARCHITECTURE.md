...
- `runCommand(args, cwd, specDir)`（`src/run/command.js:242`）：参数解析总入口，识别
...
- `runAutoMode(...)`（`src/run/command.js:1901`）：自动模式，连续跑
...
- 审批链路：`GET /api/changes/{name}/approval` 查询（`sync.js:36`，`checkApproval`）+ `approve` / `reject`
  入口（`sync.js:2298`，共用 `_submitApproval`，端点契约 TBD-hub-api）。
