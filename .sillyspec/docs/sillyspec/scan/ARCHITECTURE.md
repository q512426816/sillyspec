...
- `runCommand(args, cwd, specDir)`（`src/run/command.js:236`）：参数解析总入口，识别
...
- `runAutoMode(...)`（`src/run/command.js:1234`）：自动模式，连续跑
...
| 文件 | 职责 |
| --- | --- |
| `src/progress.js` | `ProgressManager` facade（`src/progress.js:273` 类声明）：持久化核心（_ensureDB / read / _write 本体留在 facade），其余按组 delegate 到子模块 |
...
- **`SyncManager`**（`src/sync.js:2`）：独立于 ProgressManager，由 `run/` 与 `index.js` 调用。
...
- 审批链路：`GET /api/changes/{name}/approval` 查询（`sync.js:36`，`checkApproval`）+ `approve` / `reject`
  入口（`sync.js:2220`，共用 `_submitApproval`，端点契约 TBD-hub-api）。
