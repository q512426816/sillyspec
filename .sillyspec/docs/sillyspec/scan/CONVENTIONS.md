...
**隐形规则**：文档引用源码一律带实测行号（`worktree.js:303` 形态）；改源码后同步受影响文档的行号引用；失效数走 ratchet 门只许减少不许增加（基线 `.sillyspec/docs-check-baseline`）。
...
- **数字解析**：优先 `parseInt(x, 10)`（显式基数，见 `worktree.js:303`），NaN 判断用 `Number.isNaN` 而非全局 `isNaN`。
