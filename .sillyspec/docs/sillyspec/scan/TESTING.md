...
- **known_failures 预存豁免**（`src/verify-postcheck.js:81` `extractKnownFailures` + `test/verify-postcheck-known-failures.test.mjs`）：local.yaml 可声明 `known_failures: [...]` 豁免清单，verify 对账时预存失败不阻断，但豁免 0 命中会强制人工出口（防豁免清单烂尾）。
