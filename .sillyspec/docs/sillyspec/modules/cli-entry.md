...
见 `cli-entry.changelog.md`——历史条目已迁出；新条目直接追加 sidecar，勿写回本卡。
...
- 2026-09-08 | 2026-09-08-docs-fix-capability | docs 分派双形态（--from/--to 走 docs-migrate 新语义、无 flag 落回旧 migrateDocs 兼容）；docs check 白名单加 --no-exempt；FR-5 报告段 console.error→console.log（报告 stdout/诊断 stderr，gate 零改动面）；usage 补 docs migrate/--no-exempt 说明
