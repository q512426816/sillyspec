...
消费方（scope-audit 命令 / execute--done / verify--done / archive--confirm / quick--done 四注入）只 import 本三导出（computeChangeScopeAudit/renderScopeAuditTable/collectNumstatByPath），禁止自研采集（D-003）；getFileDiff 为 scope-audit 命令 --file 出口专用（index.js 动态 import）。跨仓 actual 采集唯一源 = collectRepoActual 共享内核（`src/cross-repo-reconcile.js:200`，见上方契约/评审族条目）。冻结重放/双 map 仓消歧走模块内 `pickModuleMapProject`（内部 helper 非导出）：按样本文件对候选 _module-map.yaml 归属得分唯一最高选项目，零归属样本平分 → degraded（与 computeGateProfile.unmappedFiles 同款归属口径）。
