...
| `src/docs-gate.js` | docs check 的 ratchet 门：失效数 ≤ 基线（`.sillyspec/docs-check-baseline`）即过、超基线拦——不管历史存量只拦增量；首次须显式 `--init-baseline`（fail-closed 不悄悄合法化存量）；exit 0 过 / 1 拦 / 2 配置或 IO 错误。2026-09-15 起超基线分支实测 origin/main 兜底（临时 detach worktree 跑 docs check：未劣于远端放行/劣于拦+双参考值/实测失败 fail-open 回拦，坑 docs-gate-stale-baseline）；2026-09-17 docs-bracket-reanchor 起该陈旧分支自动重锚（已实测不劣于远端即 writeBaseline(current)+消息披露，棘轮只紧不松；checkOpts paths/skip/keywordAssert/crossRepoRoots 四键一次性覆盖守卫不写盘，local.yaml 持久口径不拦；返回面增 reanchored，快路径零远端成本/首次立线/真增量语义不变）——docs-gate 从纯读基线变为该分支条件写基线一处 |
