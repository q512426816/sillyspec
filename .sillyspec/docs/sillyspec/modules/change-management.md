...
## 注意事项（quicklog 收尾口径，2026-09-08 用户反馈①② / 2026-09-10 软归属）
...
- **软归属（2026-09-10 用户反馈）**：`--file-notes` 括注只落声明文件，同模块测试文件漏声明时审计行有提示但文件行不自动补齐（连续两批手工核对）。现 `flipEntryInContent` 接 `softFiles`（auditQuickCompletion 经 `matchSameModuleTestFiles`（`run/shared.js`，stem 相等或「声明 stem+分隔符」前缀）判定的窗口内未声明测试文件），补入文件行 bullet 带 `（软归属·同模块测试，未声明）` 括注；`--file-notes` 已显式括注的不重复加；无 fileNotes 时单行格式升级 bullet（标记放不进单行）。审计面拆分：软归属文件单列 `🔍 软归属：…已补入文件行（若属并行会话改动请手工剔除）`，`⚖️ 归属切分` 只列真未知——防两行自相矛盾。**硬归属口径不变**（声明即归属，2026-08-18 误归属修复）：softTestFiles 不进 attributedFiles，错认可由 🔍 行追溯剔除。既有边界：全新未跟踪目录 porcelain 折叠成 `?? dir/` 目录 token 时拿不到文件级路径，软归属无从匹配（折叠是审计既有口径）。
