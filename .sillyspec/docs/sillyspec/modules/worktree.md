...
# worktree
> 最后更新：2026-09-14
> 最近变更：2026-09-14-change-ownership-guards（放行相交过滤 D-003@v1：review 声明只承认与 allow 面相交文件，外来声明剔除进 violations + `result.reviewOverdeclaredFiles`；applyWorktree checkOnly 被归档收口门复用探测未 apply 交付面——接线见 runtime 卡）/ 2026-09-14-apply-conflict-hardening（merge 写回批末 git add + 三出口统一 writeApplyManifest + rescue # 指引行 + guard 相交预检三分支/--force/autoApply）/ ql-20260908-008（meta.json BOM 容错 + apply allowlist 平台模式读 specRoot——resolveApplyAllowSet/collectReviewDeclaredFiles 加 specBase/runtimeRoot 参 + 指针静默回退） / 2026-08-19-reopen-and-execute-batch-guard（W3 apply 锚点默认 merge-base + --base 回退 + 冲突列表 stderr 解析）/ 2026-08-16-state-split-fixes（#2 applyByMerge merge 前预对齐 baseline 并行文件：preAlignBase
...
| 函数/常量 | 说明 | 参数 |
|-----------|------|------|
| `applyWorktree(changeName, { cwd, checkOnly?, merge?, base?, force?, autoApply? })` | 将 worktree 变更应用到主工作区；允许集 = `resolveApplyAllowSet`（design §6 ∪ plan task allowed_paths）；主干**已提交**推进交 `--3way` 自动三路合并；`merge=true` 时先 `preAlignBaselineToMain`（2026-08-16-state-split-fixes #2，D-002@v1：baseline 含 main 已推进并行文件 → merge 前 checkout main 版 + 对齐 commit，消除 baseline vs main 冲突主因；四条件过滤保交付文件/dirty 不受伤，失败降级原 merge），未提交 dirty 拦截引导 commit/stash（判**排除规则下当前是否有未提交 dirty**，非比对启动时 baselineHash——主仓 dirty→clean 后不再死锁）；`merge=true` 显式走 git merge 兜底（D-001）；`base='baseline'` 回退旧锚点（默认 `merge-base`）；apply 文件经 `filterDeliverableFiles` 精细化过滤；dirty 拦截 fail-loud 保留，附加逐文件 cp rescue 指令（返回值新增 additive `rescueCommands`/`deletedFiles`；step3.5 前移 hashMismatch 计算，保 step4.5 拦截时 EXCLUDE-MISMATCH 可用）**+ --3way 冲突列表双源（stderr 解析 ∪ status 探测）**。**guard 相交预检（2026-09-14-apply-conflict-hardening，FR-03/D-002）**：一切主仓写动作之前，`collectActiveQuickGuardFiles`（src/quicklog.js）活跃 quick 会话 guard.allowedFiles ∩ 本次落盘集（changedFiles∪resolvePatchFiles）非空时三分支——`force:true`（CLI `--force`）放行 + `result.overlapForced` 留痕（审计可见）；`autoApply:true`（index.js assess 自动入口）软跳过（`result.overlapSkipped` + warning 指引，不抛错、零落盘）；其余 fail-closed 抛结构化错误（会话×文件对清单 + 串行化/--force 指引，index.js 走 exit 1）；空集/无声明零行为变化。成功尾声统一 `writeApplyManifest` 三出口落盘 apply-manifest.json（见下行），`result.applyManifest` 透出落点 | `changeName, { cwd, checkOnly?, merge?, base?, force?, autoApply? }` |
...
| `resolveApplyAllowSet(projectRoot, changeName)` | 解析 apply 允许文件集 = design.md §6 文件变更清单（`keepSillyspecDocs=true`，模块文档 `.sillyspec/docs/` 也进清单——默认 change-list 跳过全部 `.sillyspec/` 会导致模块文档 apply 永远缺清单）∪ 所有 task-*.md 的 allowed_paths（测试/产物文件设计常漏列但 task 已含，union 后不误拦；越界文件仍拦）。跨仓双护（2026-09-12 驾驭第十三批，troubleshooting §59）：①safeRemoveWorktreeDir 加 isCrossWorktreeDir 守卫（isCross meta ∨ <change>--<repoKey> 命名）——主仓清理面（cleanup/doctor/幽灵）一律拒删跨仓目录，cleanupCrossWorktrees 显式路径 allowCross 放行；②ensureCrossWorktrees 跳过 dirty baseline overlay/checkpoint（跨仓模型=子代理直接 commit 分支，主副本 WIP 含并行外来文件不进分支；锚点恒=跨仓仓 HEAD）。EXCLUDE-DIRTY 自动三方合并（坑 apply-dirty-block-no-merge，2026-09-10 驾驭小结第六批③）：mergeDirtyOverlapThreeWay——base=deliverableBase 锚点版 ours=主仓脏版 theirs=worktree 版，git merge-file clean 才写回（两侧并集，mergedDirtyFiles 审计），冲突维持原拦截+rescue（不留半合并现场），checkOnly 只读不试；4.5/5a 拦截口径同步引用合并残余集。变更目录读侧归档回退（坑 apply-archived-evidence-recycled，2026-09-10 驾驭小结第二批①）：活跃 `changes/<name>/` 缺 design.md 而 `changes/archive/<name>/` 在（归档后补 apply 场景——archive 有意保留未 apply worktree 却把变更目录 rename 走）→ design/tasks 从归档目录读（`resolveActiveOrArchiveChangeDir` 内部函数，assess 段 incidental/review 声明豁免集合同口径），allow 集不再恒空误拦整批 | `projectRoot, changeName` |
...
| `mergeDirtyOverlapThreeWay({ projectRoot, worktreePath, baseHash }, overlapFiles)` | EXCLUDE-DIRTY 脏重叠自动三方合并（base=deliverableBase 锚点版，ours=主仓脏版，theirs=worktree 版，git merge-file clean 才写回两侧并集）；**2026-09-14-apply-conflict-hardening（FR-01）批末统一 `git add -- <显式 pathspec>`**（chunkPaths 分批）——本写回点曾是全链路唯一未暂存的自动写点（未暂存对工作区级 git restore/clean 零免疫，§64 事故实际缺口）；fail-open：add 失败不抛（合并成果保留）+ warn + 返回 `stagedOk:false`（manifest 侧对该批回退 readFile+CRLF 归一 approx 指纹口径）；返回 `{merged, conflicts, stagedOk}` | `{ projectRoot, worktreePath, baseHash }, overlapFiles` |
| `writeApplyManifest({ projectRoot, specBase?, changeName, baseHash?, files })`（内部） | apply 成功尾声写 `<changeDir>/apply-manifest.json`（resolveActiveOrArchiveChangeDir 归档感知；已存在覆盖——重放 apply 以最新为准；files 为空跳过不写）。**三出口统一**（2026-09-14-apply-conflict-hardening，FR-02/D-001）：①patch 主路径成功尾声（patch∪merge 写回面并集）②patch 为空但 merge 写回面非空的变体出口 ③applyByMerge 成功点（显式 `--merge` 与 ENOBUFS 自动降级两条提前 return）。schema `{schemaVersion:1, change, appliedAt, baseHash, files:[{path,sha256,approx?}]}`；sha256 一律取 **staged blob 内容**（`git show :<path>`，LF 规范态）与 doctor 漂移检测同基，staged 不可用回退磁盘 CRLF→LF 归一并标 `approx:true`，双无（删除面）不进 manifest；CLI 全权写，agent 勿手改 | `{ projectRoot, specBase?, changeName, baseHash?, files }` |
| `generateRescueCommands` 指引行（D-003） | rescue 人工落地路径不进 manifest 也不自动暂存——2026-09-14-apply-conflict-hardening 起 commands 末尾追加一行 `# 落地后立即 git add -- …` 锁定指引（注释形态整块复制仍是合法 shell；commands 为空不加；cpFileCount/excludedCount 口径不变） | 同上行 |
| `result.reviewOverdeclaredFiles`（applyWorktree 返回字段） | **review 声明相交过滤（D-003@v1，2026-09-14-change-ownership-guards task-03 / FR-02）**：step 3b——review.json changedFiles 声明只承认与 allow 面（`resolveApplyAllowSet` 并集：design §6 清单 ∪ 各 task target_files/allowed_paths ∪ linked-change 声明）相交的文件（`pathMatches` 容差）；不相交的外来声明剔除出放行面，主仓切片进本字段 + violations 报告行（Gate1 拦截时分流「review 声明了越权文件」嫌疑标注 vs 完全未声明两族）+ 审计 warning（troubleshooting §65 事件②：并行会话在途文件混入声明不再经该通道放行——admission 增量归零，通道保留作审计报告位）。跨仓声明不进 main 切片（repo 键分流）；该 repo 无 design/任务卡清单时无相交可判不校验。主仓与跨仓（worktree-apply.js 内 reviewAdmittedFiles 两路径）同口径 | applyWorktree 返回值（字符串数组，无外来声明时 undefined） |
...
3. **应用流**: applyWorktree → 检查 worktree 存在 → git diff 生成文件列表 → **guard 相交预检（活跃 quick 会话 allowedFiles ∩ 落盘集，三分支：--force 放行留 overlapForced / autoApply 软跳过 / 否则 fail-closed）** → 冲突检测 → 生成补丁 → git apply（--3way；merge 写回批末 git add 进暂存）→ 处理未跟踪文件 → **成功尾声 writeApplyManifest 落 apply-manifest.json 指纹（三出口统一）**
...
- **归档收口门复用 checkOnly 探测（2026-09-14-change-ownership-guards，D-002@v1 关联）**：run/complete-handlers.js archiveChangeDirectory 归档移动前调 `applyWorktree(name, { checkOnly: true })` 探测未 apply 交付面（只读零写盘），非空即阻断归档（`--skip-apply` 显式跳过留痕）——堵「归档完成但交付物未进主仓」悬空态（troubleshooting §65 事件①放大器）。门与所有权接线归 runtime 卡；本卡只登记 applyWorktree checkOnly 的这一新消费方
